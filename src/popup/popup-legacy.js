// This is the existing `popup.js` logic with a tiny change:
// - removed DOMContentLoaded auto-run
// - exported `initPopup()` so React can render DOM first, then bind listeners.
...
// Legacy DOM implementation kept for reference only.
// The popup now uses React + Zustand + Axios + HashRouter.
export async function initPopup() {
  console.warn("[popup-legacy] initPopup() is deprecated and should not be called.");
}
  ...
  window.chrome.storage.local.get(
    ...
    (result) => {
      if (result.user) {
        ...
        if (autoSyncToggle) autoSyncToggle.checked = Boolean(result.autoSyncToggle);
...
async function extractFromActiveTab() {
  ...
  if (!isAStuFromUrl && !portalCheck?.isASTU) {
    throw new Error("Open ASTU e-Student Academic History page before syncing.");
  }
...
async function scrapeViaInjectedFunction(tabId) {
  ...
  const [{ result } = {}] = await window.chrome.scripting.executeScript({
    ...
    func: () => {
      ...
      try {
        const sections = Array.from(document.querySelectorAll("#transcript .mb-4"));
        const semesters = [];
        const allCourses = [];

        sections.forEach((section) => {
          const academicYear = clean(section.querySelector(".card-header h6")?.textContent);
          const semesterLabel = clean(section.querySelector(".card-header p")?.textContent);
          if (!academicYear || !semesterLabel) return;

          const rows = Array.from(section.querySelectorAll("table tbody tr")).filter(
            (row) => row.children.length >= 5,
          );
          ...
          const courses = rows
            .map((row) => {
              ...
              return { code, title, credit_hours: creditHours, grade, points };
            })
            .filter(Boolean);
        ...
        if (!semesters.length) {
          return { success: false, error: "No transcript semesters found on this page." };
        }
        ...
        return {
          ...
          data: {
            ...
            year: null,
            status: "Unknown",
            gpa: String(semesters[semesters.length - 1]?.cumulative_summary?.cgpa || ""),
            courses: allCourses,
            transcriptData: { student_id: null, semesters },
            extractedFrom: "ASTU eStudent Portal (Fallback)",
            extractedAt: new Date().toISOString(),
          },
        };
      } catch (error) {
        return { success: false, error: error?.message || "Injected scraper failed." };
      }
    },
  });

  return result || { success: false, error: "Injected scraper returned no result." };
}

function logSyncStep(step, details = {}) {
  const entry = { time: new Date().toISOString(), step, ...details };
...
function toLogSafe(value, maxLength = 900) {
  try {
    ...
    return `${asString.slice(0, maxLength)} ...[truncated ${asString.length - maxLength} chars]`;
  } catch (_error) {
...
async function processSyncData(data, prebuiltPayload = null) {
  ...
  try {
    ...
    const transcriptPayload = prebuiltPayload || buildTranscriptCreatePayload(data);
    ...
    logSyncStep("upload-response", { response: toLogSafe(uploadResponse) });
    ...
    try {
      ...
      logSyncStep("latest-response", { response: toLogSafe(latestTranscript) });
    } catch (latestError) {
      ...
      courses: validCourses.map((course) => ({
        ...
        points:
          typeof schema.grading_schema?.[course.grade] === "number"
            ? Number(
                (
                  Number(course.credits || 0) * schema.grading_schema[course.grade]
                ).toFixed(2),
              )
            : 0,
      })),
  ...
  return semesters
    .map((semester) => {
      ...
      const semesterCreditHours =
        ...
        courses.reduce((sum, course) => sum + Number(course.credit_hours || 0), 0);
...
async function recommendTranscriptConfig(transcriptPayload) {
  ...
  return apiFetch(TRANSCRIPT_RECOMMEND_API_URL, { method: "POST", body: transcriptPayload });
}

async function uploadTranscript(transcriptPayload) {
  // NOTE: kept as-is from original popup.js (uses a temporary test payload).
  const testPayload = {
    transcript_data: {
      ...
      semesters: [
        {
          ...
          year_level: "2",
          courses: [
            { code: "CS201", title: "Algorithms", credit_hours: 3, grade: "A", points: 12 },
            { code: "STAT201", title: "Statistics", credit_hours: 3, grade: "B+", points: 9 },
          ],
          semester_summary: {
            credit_hours: 6,
            points: 21,
          ...
          cumulative_summary: { credit_hours: 30, points: 102, cgpa: 3.4 },
        },
      ],
    },
  };
...
async function loadTranscriptConfig(showToast) {
  try {
    ...
    if (showToast)
      setTranscriptConfigStatus("Loaded transcript config from backend.", "success");
  } catch (error) {
    if (!transcriptConfig) {
    ...
    if (showToast) {
      setTranscriptConfigStatus(error.message || "Unable to load transcript config.", "error");
    }
  }
}
...
async function loadTranscriptPresets(showToast) {
  try {
    ...
    if (showToast)
      setTranscriptConfigStatus(`Loaded ${gradingPresets.length} presets.`, "success");
  } catch (error) {
    if (showToast) {
      setTranscriptConfigStatus(error.message || "Unable to load presets.", "error");
    }
  }
}
...
function renderTranscriptConfigForm(config) {
  ...
  const gradeDisplayOrderInput = document.getElementById("gradeDisplayOrderInput");

  if (gpaScaleInput) gpaScaleInput.value = config.gpa_scale;
  if (gradingSchemaInput) gradingSchemaInput.value = JSON.stringify(config.grading_schema, null, 2);
...
function readTranscriptConfigFromForm() {
  ...
  return { gpa_scale: gpaScale, grading_schema: gradingSchema, grade_display_order: gradeDisplayOrder };
}

async function handleSaveTranscriptConfig() {
  try {
    const payload = readTranscriptConfigFromForm();
    const saved = await apiFetch(TRANSCRIPT_CONFIG_API_URL, { method: "PUT", body: payload });
    ...
    setTranscriptConfigStatus(error.message || "Unable to save transcript config.", "error");
  }
}

async function handleResetTranscriptConfig() {
  try {
    await apiFetch(TRANSCRIPT_CONFIG_API_URL, { method: "DELETE" });
    await loadTranscriptConfig(false);
    setTranscriptConfigStatus("Transcript config reset to backend defaults.", "success");
  } catch (error) {
    setTranscriptConfigStatus(error.message || "Unable to reset config.", "error");
  }
}
...
async function handleRecommendConfig() {
  try {
    const payload = buildTranscriptCreatePayload(syncedData || getMockSyncData());
    ...
    if (recommendation?.suggested_config) {
      transcriptConfig = {
        ...
        grade_display_order: recommendation.suggested_config.grade_display_order,
      };
      renderTranscriptConfigForm(transcriptConfig);
    }
    ...
    setTranscriptConfigStatus(error.message || "Unable to get recommendation.", "error");
  }
}
    ...
    try {
      ...
      if (!skipAutoLogout) {
        await performLogoutFlow({
          reason: authError.message || "Missing access token. Please login again.",
          notifyUser: true,
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const shouldForceLogout =
        isAuthError && requireAuth && !skipAutoLogout && !isSessionResetInProgress;
  ...
  if (state === "success") {
    ...
    activityList.innerHTML = `
      ...
      <div class="activity-item ${
        stage === "review" || stage === "apply-confirmed"
          ? "active"
          : applyDone
            ? "completed"
            : ""
...
function displaySyncedData() {
  ...
  if (!syncedData) {
    if (personalData) personalData.innerHTML = '<p class="empty-state">No synced data yet.</p>';
    if (coursesData) coursesData.innerHTML = '<p class="empty-state">No course data available.</p>';
    if (progressData) progressData.innerHTML = '<p class="empty-state">No progress data available.</p>';
    return;
  }
  ...
  if (progressData) {
    const completion =
      syncedData.degreeProgress?.completionPercentage || syncedData.completion || 0;
