...
function extractProfileInfo() {
  ...
  return { name: nameText, department: departmentText, year: yearText, studentId, email };
}
...
function extractTranscriptTabData() {
  ...
  const allSemesters = semesterBlocks.map((block) => parseSemesterBlock(block)).filter(Boolean);
  ...
  return { allSemesters, uploadableSemesters, pendingSemesters, latestAcademicStatus };
}
...
function parseSemesterBlock(block) {
  ...
  const semesterCreditHours = footerInfo.semesterCreditHours ?? calculatedCreditHours;
  const semesterPoints = footerInfo.semesterPoints ?? calculatedPoints;
  const semesterSgpa =
    footerInfo.semesterSgpa ??
    (semesterCreditHours > 0 ? round2(semesterPoints / semesterCreditHours) : null);

  const cumulativeCreditHours = footerInfo.cumulativeCreditHours ?? semesterCreditHours;
  const cumulativePoints = footerInfo.cumulativePoints ?? semesterPoints;
  const cumulativeCgpa =
    footerInfo.cumulativeCgpa ??
    (cumulativeCreditHours > 0 ? round2(cumulativePoints / cumulativeCreditHours) : null);
...
function parseSemesterHeading(heading) {
  ...
  return { academicYear: cleanText(match[1]), semester: cleanText(match[2]), yearLevel: cleanText(match[3] || "") || null };
}
...
function parseCourseRow(row) {
  ...
  if (creditHours === null || creditHours <= 0) return null;

  return { code, title, credit_hours: creditHours, grade: grade || "-", points };
}
...
function parseFooter(tfoot) {
  ...
  const academicStatusMatch = cumulativeRowText.match(/Academic\s+Status\s*:\s*([^\n]+)/i);
  ...
  return {
    ...
    academicStatus: academicStatusMatch ? cleanText(academicStatusMatch[1]) : null,
  };
}
...
function extractGpaSummary() {
  ...
  return {
    academicStatus: extractByPattern(text, /Academic\s+status\s*:\s*([^\n]+)/i) || null,
    semesterGpa: parseNumeric(extractByPattern(text, /Semester\s+GPA\s*:\s*([\d.\-]+)/i)),
    cumulativeGpa: parseNumeric(extractByPattern(text, /Cumulative\s+GPA\s*:\s*([\d.\-]+)/i)),
  };
}
...
function scrapePageData() {
  ...
  return {
    studentId: extractByPattern(text, /(?:Student\s*ID|ID\s*No)\s*[:\-]\s*([A-Za-z0-9\/-]+)/i) || "STU-UNKNOWN",
    name: extractByPattern(text, /Name\s*[:\-]\s*([^\n]+)/i) || "Unknown Student",
    department: extractByPattern(text, /Department\s*[:\-]\s*([^\n]+)/i) || "Unknown Department",
    year: extractByPattern(text, /(First|Second|Third|Fourth|Fifth|Sixth)\s+Year/i) || "Unknown Year",
    status: "Unknown",
    courses: [],
